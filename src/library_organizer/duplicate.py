...
def _collect_files(temporary_dir: Path) -> list[Path]:
    """
    Collect file paths from directory (recursive), skipping excluded extensions.

    This now walks all subdirectories under temporary_dir, while keeping
    extension-skip behavior consistent with main.py.
    """
    ...
    try:
        ...
        logger.warning("Cannot iterate directory %s: %s", temporary_dir, exc)
    return paths
...
def find_duplicates(
    ...
    """
    progress_report = progress_callback or (lambda c, t, p, ph: None)
    paths = _collect_files(temporary_dir)
    total_files = len(paths)
    for i in range(1, total_files + 1):
        if i % 5 == 0 or i == total_files:
            progress_report(i, total_files, "Scanning  ", "scanning")
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for future in as_completed(futures):
            ...
            if done % 5 == 0 or done == total_files:
                progress_report(done, total_files, "Pass 1    ", "pass_1")
            if result is None:
                continue
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for future in as_completed(futures):
                ...
                if done % 5 == 0 or done == total_candidates:
                    progress_report(done, total_candidates, "Pass 2    ", "pass_2")
                if result is None:
                    continue
    ...
    progress_report(1, 1, "Writing   ", "writing")
